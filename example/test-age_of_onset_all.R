...
h2 <- 0.5
N <- 2.5e4
prev <- 0.3#c(0.08, 0.02)
downsample_frac <- .6 #1 for no downsampling.
...
father_age = sample(1:len_thr, N, replace = TRUE)
mother_age = sample(1:len_thr, N, replace = TRUE)
child_age = sample(1:len_thr, N, replace = TRUE)

father_status <- assign_status(father_full, thr)

mother_status <- assign_status(mother_full, thr)
